...
// ── VINCULAR FILL ─────────────────────────────────────────────────────────────
// El pare introdueix l'àlies del fill per vincular-s'hi
...
async function vincularFill(req, res) {
  ...
  try {
    ...
    await pool.query(
      'INSERT IGNORE INTO familia (pare_id, fill_id) VALUES (?, ?)',
      [pareId, fill.id]
    );

    return res.json({ ok: true, fill: { id: fill.id, nom: fill.nom, alias: fill.alias } });
  } catch (err) {
  ...
  try {
    const [fills] = await pool.query(
      `SELECT u.id, u.alias, u.nom, u.rang, u.nivell, u.xp_total, u.xp_setmana,
       ...
       WHERE f.pare_id = ? AND u.actiu = TRUE
       ORDER BY f.creat_at ASC`,
      [pareId]
    );

    if (fills.length === 0) return res.json([]);

    // Progrés de nodes per a cada fill
    const fillIds = fills.map(f => f.id);
    const [nodes] = await pool.query(
      `SELECT usuari_id, estat FROM progres_nodes WHERE usuari_id IN (?)`,
      [fillIds]
    );
    ...
    // Punts febles (matèries amb menys progrés)
...
async function informeFill(req, res) {
  ...
  try {
    // Verificar que el fill pertany a aquest pare
    const [check] = await pool.query(
      'SELECT id FROM familia WHERE pare_id = ? AND fill_id = ?',
      [pareId, fillId]
    );
...
module.exports = { soloPare, vincularFill, desvincularFill, getFills, informeFill };
